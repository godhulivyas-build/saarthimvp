  ...
  return (
    <div className="fixed inset-0 z-[70] flex flex-col justify-end sm:justify-center bg-black/45 p-0 sm:p-4">
      <Card className="w-full max-w-md mx-auto rounded-b-none sm:rounded-3xl max-h-[85vh] flex flex-col p-0 overflow-hidden">
        <div className="flex items-center justify-between px-4 py-3 border-b border-[var(--sarthi-outline-soft)]">
          <div>
            <p className="font-extrabold sarthi-headline text-[var(--sarthi-primary)]">{crop}</p>
            <p className="text-xs text-[var(--sarthi-on-surface-variant)]">{farmerName}</p>
          </div>
          <button type="button" className="p-2 rounded-xl hover:bg-[var(--sarthi-surface-low)]" onClick={onClose} aria-label="Close">
            <X className="w-5 h-5" />
          </button>
        </div>
        <div className="flex-1 overflow-y-auto px-4 py-3 space-y-2 bg-[var(--sarthi-surface-low)] min-h-[200px]">
          {loading ? (
            ...
            rows.map((m) => (
              <div
                ...
                className={`max-w-[88%] rounded-2xl px-3 py-2 text-sm ${
                  m.role === 'buyer' ? 'ml-auto bg-[var(--sarthi-primary)] text-white rounded-br-md' : 'mr-auto bg-white border border-[var(--sarthi-outline-soft)] rounded-bl-md'
                }`}
              >
                {m.text}
              </div>
        ...
        <form onSubmit={send} className="p-4 border-t border-[var(--sarthi-outline-soft)] space-y-2 bg-white">
